...
type
  ...
  protected
    ...
    function GenerateCanonicalRequest(AVerb, URI: string; APayload: TStream; AHeaders, AQueryValues: TStrings): string; overload;
...
begin
  ...
  else if ARegionStr = C_RGN_EU_NORTH_1 then Result := awsEuNorth1
  else if ARegionStr = C_RGN_EU_SOUTH_1 then Result := awsEuSouth1
  else if ARegionStr = C_RGN_EU_WEST_1 then Result := awsEuWest1
  else if ARegionStr = C_RGN_EU_WEST_2 then Result := awsEuWest2
  else if ARegionStr = C_RGN_EU_WEST_3 then Result := awsEuWest3
  else if ARegionStr = C_RGN_US_EAST_1 then Result := awsUsEast1
  else if ARegionStr = C_RGN_US_EAST_2 then Result := awsUsEast2
  else if ARegionStr = C_RGN_US_WEST_1 then Result := awsUsWest1
  else if ARegionStr = C_RGN_US_WEST_2 then Result := awsUsWest2
  else raise Exception.CreateFmt('Unknown AWS region: %s', [ARegionStr]);
end;
  ...
  begin
    ...
    begin
      if ICount = 0 then
        ...
        Result := Result + '&';
      Result := Result + ParamEncode(AParams.Names[ICount])+'='+ParamEncode(AParams.ValueFromIndex[ICount]);    end;
  end;
...
function TksAwsBaseService.GenerateCanonicalRequest(AVerb, URI: string; APayload: TStream; AHeaders, AQueryValues: TStrings): string;
var
  ICount:
  integer;
  AHash: string;
begin
  ...
  Result := AVerb +C_LF;
  Result := Result + URI +C_LF;
  if AQueryValues <> nil then
  begin
    (AQueryValues as TStringList).Sort;
    for ICount := 0 to AQueryValues.Count-1 do
    begin
      Result := Result + ParamEncode(AQueryValues.Names[ICount])+'='+ParamEncode(AQueryValues.ValueFromIndex[ICount]);
      if ICount < AQueryValues.Count-1 then Result := Result + '&';
    end;
  end;
  Result := Result + C_LF;;
  for ICount := 0 to AHeaders.Count-1 do
    Result := Result + LowerCase(AHeaders.Names[ICount])+':'+Trim(AHeaders.ValueFromIndex[ICount])+C_LF;
  Result := Result + C_LF;
  for ICount := 0 to AHeaders.Count-1 do
  begin
    Result := Result + LowerCase(AHeaders.Names[ICount]);
...
begin
  AHeaders.Values['host'] := Trim(AHost);
...
begin
  ...
  try
    ...
    for ICount := 0 to AHeaders.Count-1 do
    begin
      ADelimitedHeaders := ADelimitedHeaders + LowerCase(AHeaders.Names[ICount]);
      if ICount < AHeaders.Count-1 then
        ADelimitedHeaders := ADelimitedHeaders + ';';
    end;
    AAmzDate := FormatDateTime(C_AMZ_DATE_FORMAT, ANowUtc, TFormatSettings.Create('en-US'));

    ACanonical := GenerateCanonicalRequest(AVerb, APath, AContent, AHeaders, AParams);
    AStringToSign := C_HASH_ALGORITHM +C_LF +
                     AAmzDate +C_LF+
                     FormatDateTime(C_SHORT_DATE_FORMAT, ANowUtc, TFormatSettings.Create('en-US')) +'/'+ ARegion +'/'+ GetServiceName +'/aws4_request' +C_LF+
    ...
    AAuthHeader := C_HASH_ALGORITHM+' Credential='+FAccessKey+'/'+
                   FormatDateTime(C_SHORT_DATE_FORMAT, ANowUtc, TFormatSettings.Create('en-US'))+'/'+
                   ARegion+'/'+
                   GetServiceName+'/'+
